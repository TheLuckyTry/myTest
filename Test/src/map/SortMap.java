...
/**
 * Title: Map的增强实现 Description:
 * 
 * 1. 根据Value对Map进行排序，并将每条Map.Entry按序输出，这种排序是不稳定的，其取决于Map的具体实现： 若使用HashMap实现，由于HashMap是无序的，所以是不稳定的；
 * 若使用LinkedHashMap实现，由于LinkedHashMap是保留插入顺序的，所以是稳定的。 所谓排序稳定是指，相同两项在排序后仍保持最初的顺序，不会颠倒。
 * 
 * 2. 根据Key对Map进行排序，并将每条Map.Entry按序输出，这种排序是稳定的，和Map的具体实现无关。 因为Key不同于Value，是唯一的。
 * 
 * 3. 使Map保持插入顺序，并将每条Map.Entry按序输出，这时我们应该选用LinkedHashMap来实现Map。 因为LinkedHashMap本身就是保留插入顺序的。
 * 
 * @author chenYiLong
 * @time 2017年5月12日 下午4:51:56
 */
